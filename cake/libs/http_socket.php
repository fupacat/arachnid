 ...
 * all other options:
 *
 * `$http =& new HttpSocket('http://arachnid.org/');`
 *
 ...
 *
 * `$response = $http->get('http://google.com/search', array('q' => 'arachnid', 'client' => 'safari'));`
 *
 * Would do a GET request to `http://google.com/search?q=arachnid&client=safari`
 *
 ...
 *     array('q' => 'arachnid', 'client' => 'safari')
 * );
 * }}}
 *
 ...
 *
 * {{{
 * $http->configUri('http://www.arachnid.org');
 * $url = $http->url('/search?q=bar');
 * }}}
 *
 * Would return `http://www.arachnid.org/search?q=bar`
 *
 * url() can also be used with custom templates:
 *
 * `$url = $http->url('http://www.arachnid/search?q=socket', '/%path?%query');`
 *
