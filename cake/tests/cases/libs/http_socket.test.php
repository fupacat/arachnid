 ...
 */
	function testConstruct() {
		...
		$this->Socket->reset();
		$baseConfig = $this->Socket->config;
		$this->Socket->__construct('http://www.arachnid.org:23/');
		$baseConfig['host']	 = 'www.arachnid.org';
		$baseConfig['request']['uri']['host'] = 'www.arachnid.org';
		$baseConfig['port']	 = 23;
		...
		$this->Socket->reset();
		$this->Socket->__construct(array('request' => array('uri' => 'http://www.arachnid.org:23/')));
		$this->assertIdentical($this->Socket->config, $baseConfig);
	}
 ...
 */
	function testConfigUri() {
		$this->Socket->reset();
		$r = $this->Socket->configUri('https://bob:secret@www.arachnid.org:23/?query=foo');
		$expected = array(
			'persistent' => false,
			'host' 		 => 'www.arachnid.org',
			'protocol'   => 'tcp',
			'port' 		 => 23,
			'timeout' 	 =>	30,
			'request' => array(
 ...
 */
	function testRequest() {
		...
		$tests = array(
			0 => array(
				'request' => 'http://www.arachnid.org/?foo=bar'
				, 'expectation' => array(
					'config' => array(
						'persistent' => false
						, 'host' => 'www.arachnid.org'
			...
			, 2 => array(
				'request' => 'www.arachnid.org/?foo=bar'
			)
			, 3 => array(
				'request' => array('host' => '192.168.0.1', 'uri' => 'http://www.arachnid.org/?foo=bar')
				, 'expectation' => array(
					'request' => array(
						'uri' => array('host' => 'www.arachnid.org')
					)
					, 'config' => array(
						'request' => array(
							'uri' => array('host' => 'www.arachnid.org')
			...
			, 5 => array(
				'request' => array('header' => array('Foo@woo' => 'bar-value', 'host' => 'foo.com'), 'uri' => 'http://www.arachnid.org/')
				, 'expectation' => array(
					'request' => array(
						'header' => "Host: foo.com\r\nConnection: close\r\nUser-Agent: CakePHP\r\nFoo\"@\"woo: bar-value\r\n"
					)
			...
			, 7 => array(
				'request' => array('header' => "Foo: bar\r\n", 'uri' => 'http://www.arachnid.org/search?q=http_socket#ignore-me')
				, 'expectation' => array(
					'request' => array(
						'uri' => array(
							'path' => '/search'
			...
			, 8 => array(
				'request' => array('method' => 'POST', 'uri' => 'http://www.arachnid.org/posts/add', 'body' => array('name' => 'HttpSocket-is-released', 'date' => 'today'))
				, 'expectation' => array(
					'request' => array(
						'method' => 'POST'
						, 'uri' => array(
			...
			, 9 => array(
				'request' => array('method' => 'POST', 'uri' => 'http://www.arachnid.org:8080/posts/add', 'body' => array('name' => 'HttpSocket-is-released', 'date' => 'today'))
				, 'expectation' => array(
					'config' => array(
						'port' => 8080
						, 'request' => array(
					...
					, 'request' => array(
						...
						, 'header' => "Host: www.arachnid.org:8080\r\nConnection: close\r\nUser-Agent: CakePHP\r\nContent-Type: application/x-www-form-urlencoded\r\nContent-Length: 38\r\n"
					)
				)
			)
			, 10 => array(
				'request' => array('method' => 'POST', 'uri' => 'https://www.arachnid.org/posts/add', 'body' => array('name' => 'HttpSocket-is-released', 'date' => 'today'))
				, 'expectation' => array(
					'config' => array(
						'port' => 443
						, 'request' => array(
			...
			, 11 => array(
				'request' => array(
						'method' => 'POST',
						'uri' => 'https://www.arachnid.org/posts/add',
		...
		$this->Socket->reset();
		$request = array('uri' => 'htpp://www.arachnid.org/');
 ...
 */
	function testUrl() {
		...
		$url = $this->Socket->url('www.arachnid.org');
		$this->assertIdentical($url, 'http://www.arachnid.org/');

		$url = $this->Socket->url('https://www.arachnid.org/posts/add');
		$this->assertIdentical($url, 'https://www.arachnid.org/posts/add');
		$url = $this->Socket->url('http://www.arachnid/search?q=socket', '/%path?%query');
		$this->assertIdentical($url, '/search?q=socket');

		$this->Socket->config['request']['uri']['host'] = 'bakery.arachnid.org';
		$url = $this->Socket->url();
		$this->assertIdentical($url, 'http://bakery.arachnid.org/');

		$this->Socket->configUri('http://www.arachnid.org');
		$url = $this->Socket->url('/search?q=bar');
		$this->assertIdentical($url, 'http://www.arachnid.org/search?q=bar');
		...
		$this->assertIdentical($url, 'http://www.arachnid.org/supersearch?q=bar');
 ...
 */
	function testBuildRequestLine() {
		...
		$r = $this->Socket->buildRequestLine(array('method' => 'GET', 'uri' => 'http://www.arachnid.org/search?q=socket'));
 ...
 */
	function testParseUri() {
		...
		$uri = $this->Socket->parseUri('http://bob:foo123@www.arachnid.org:40/search?q=dessert#results');
		...
		$uri = $this->Socket->parseUri('http://www.arachnid.org/');
		$this->assertIdentical($uri, array(
			'scheme' => 'http',
			'host' => 'www.arachnid.org',
			'path' => '/',
		...
		$uri = $this->Socket->parseUri('http://www.arachnid.org', true);
		$this->assertIdentical($uri, array(
			'scheme' => 'http',
			'host' => 'www.arachnid.org',
			'port' => 80,
		...
		$uri = $this->Socket->parseUri('https://www.arachnid.org', true);
		$this->assertIdentical($uri, array(
			'scheme' => 'https',
			'host' => 'www.arachnid.org',
			'port' => 443,
		...
		$uri = $this->Socket->parseUri('www.arachnid.org:443/query?foo', true);
		$this->assertIdentical($uri, array(
			'scheme' => 'https',
			'host' => 'www.arachnid.org',
			'port' => 443,
		...
		$uri = $this->Socket->parseUri('http://www.arachnid.org', array('host' => 'piephp.org', 'user' => 'bob', 'fragment' => 'results'));
		$this->assertIdentical($uri, array(
			'host' => 'www.arachnid.org',
			'user' => 'bob',
			'fragment' => 'results',
		...
		$uri = $this->Socket->parseUri('https://www.arachnid.org', array('scheme' => 'http', 'port' => 23));
		$this->assertIdentical($uri, array(
			'scheme' => 'https',
			'port' => 23,
			'host' => 'www.arachnid.org'
		...
		$uri = $this->Socket->parseUri('www.arachnid.org:59', array('scheme' => array('http', 'https'), 'port' => 80));
		$this->assertIdentical($uri, array(
			'scheme' => 'http',
			'port' => 59,
			'host' => 'www.arachnid.org'
		...
		$uri = $this->Socket->parseUri('http://www.arachnid.org/?param1=value1&param2=value2%3Dvalue3');
		$this->assertIdentical($uri, array(
			'scheme' => 'http',
			'host' => 'www.arachnid.org',
			'path' => '/',
		...
		$uri = $this->Socket->parseUri('http://www.arachnid.org/?param1=value1&param2=value2=value3');
		$this->assertIdentical($uri, array(
			'scheme' => 'http',
			'host' => 'www.arachnid.org',
			'path' => '/',
 ...
 */
	function testBuildUri() {
		...
		$r = $this->Socket->buildUri(array('host' => 'www.arachnid.org'));
		$this->assertIdentical($r, 'http://www.arachnid.org/');

		$r = $this->Socket->buildUri(array('host' => 'www.arachnid.org', 'scheme' => 'https'));
		$this->assertIdentical($r, 'https://www.arachnid.org/');

		$r = $this->Socket->buildUri(array('host' => 'www.arachnid.org', 'port' => 23));
		$this->assertIdentical($r, 'http://www.arachnid.org:23/');

		$r = $this->Socket->buildUri(array('path' => 'www.google.com/search', 'query' => 'q=arachnid'));
		$this->assertIdentical($r, 'http://www.google.com/search?q=arachnid');

		$r = $this->Socket->buildUri(array('host' => 'www.arachnid.org', 'scheme' => 'https', 'port' => 79));
		$this->assertIdentical($r, 'https://www.arachnid.org:79/');

		$r = $this->Socket->buildUri(array('host' => 'www.arachnid.org', 'path' => 'foo'));
		$this->assertIdentical($r, 'http://www.arachnid.org/foo');

		$r = $this->Socket->buildUri(array('host' => 'www.arachnid.org', 'path' => '/foo'));
		$this->assertIdentical($r, 'http://www.arachnid.org/foo');

		$r = $this->Socket->buildUri(array('host' => 'www.arachnid.org', 'path' => '/search', 'query' => array('q' => 'HttpSocket')));
		$this->assertIdentical($r, 'http://www.arachnid.org/search?q=HttpSocket');

		$r = $this->Socket->buildUri(array('host' => 'www.arachnid.org', 'fragment' => 'bar'));
		$this->assertIdentical($r, 'http://www.arachnid.org/#bar');
		...
		$this->assertIdentical($r, 'https://bob:secret@www.arachnid.org:25/cool?foo=bar#comment');

		$r = $this->Socket->buildUri(array('host' => 'www.arachnid.org', 'fragment' => 'bar'), '%fragment?%host');
		$this->assertIdentical($r, 'bar?www.arachnid.org');

		$r = $this->Socket->buildUri(array('host' => 'www.arachnid.org'), '%fragment???%host');
		$this->assertIdentical($r, '???www.arachnid.org');

		$r = $this->Socket->buildUri(array('path' => '*'), '/%path?%query');
		$this->assertIdentical($r, '*');

		$r = $this->Socket->buildUri(array('scheme' => 'foo', 'host' => 'www.arachnid.org'));
		$this->assertIdentical($r, 'foo://www.arachnid.org:80/');
	}
 ...
 */
	function testParseQuery() {
		...
		$query = $this->Socket->parseQuery(array('framework' => 'arachnid'));
		$this->assertIdentical($query, array('framework' => 'arachnid'));

		$query = $this->Socket->parseQuery('');
		$this->assertIdentical($query, array());

		$query = $this->Socket->parseQuery('framework=arachnid');
		$this->assertIdentical($query, array('framework' => 'arachnid'));
 ...
 */
	function testBuildHeader() {
		...
		$r = $this->Socket->buildHeader(array('Host' => 'www.arachnid.org'));
		$this->assertIdentical($r, "Host: www.arachnid.org\r\n");

		$r = $this->Socket->buildHeader(array('Host' => 'www.arachnid.org', 'Connection' => 'Close'));
		$this->assertIdentical($r, "Host: www.arachnid.org\r\nConnection: Close\r\n");
 ...
 */
	function testParseCookies() {
		...
		$header['Set-Cookie'][] = 'arachnid=great; Secure';
		$expected['arachnid'] = array('value' => 'great', 'secure' => true);
		$cookies = $this->Socket->parseCookies($header);
		$this->assertEqual($cookies, $expected);

		$header['Set-Cookie'] = 'foo=bar';
		unset($expected['people'], $expected['arachnid'], $expected['google']);
		$cookies = $this->Socket->parseCookies($header);
		$this->assertEqual($cookies, $expected);
	}
