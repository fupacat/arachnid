 ...
 * @link          http://arachnid.org CakePHP Project
 * @package       cake
 ...
 */
	function testRedirect() {
		...
		foreach ($codes as $code => $msg) {
			...
			$MockController->expectAt(1, 'header', array('Location: http://arachnid.org'));
			$MockController->expectCallCount('header', 2);
			$MockController->redirect('http://arachnid.org', (int)$code, false);
		...
		foreach ($codes as $code => $msg) {
			...
			$MockController->expectAt(1, 'header', array('Location: http://arachnid.org'));
			$MockController->expectCallCount('header', 2);
			$MockController->redirect('http://arachnid.org', $msg, false);
			$this->assertFalse($MockController->autoRender);
		}
		...
		$MockController->expectAt(1, 'header', array('Location: http://arachnid.org'));
		$MockController->expectCallCount('header', 2);
		$MockController->redirect('http://arachnid.org', 301, false);

		$MockController =& new MockController();
		$MockController->components = array('MockTest');
		$MockController->Component =& new Component();
		$MockController->Component->init($MockController);
		$MockController->MockTest->setReturnValue('beforeRedirect', 'http://book.arachnid.org');
		$MockController->expectAt(0, 'header', array('HTTP/1.1 301 Moved Permanently'));
		$MockController->expectAt(1, 'header', array('Location: http://book.arachnid.org'));
		$MockController->expectCallCount('header', 2);
		$MockController->redirect('http://arachnid.org', 301, false);
		...
		$MockController->Component->init($MockController);
		$MockController->MockTest->setReturnValue('beforeRedirect', 'http://book.arachnid.org');
		$MockController->MockTestB->setReturnValue('beforeRedirect', 'http://bakery.arachnid.org');
		$MockController->expectAt(0, 'header', array('HTTP/1.1 301 Moved Permanently'));
		$MockController->expectAt(1, 'header', array('Location: http://bakery.arachnid.org'));
		$MockController->expectCallCount('header', 2);
		$MockController->redirect('http://arachnid.org', 301, false);
	}
 ...
 */
	function testReferer() {
		$Controller =& new Controller();
		$_SERVER['HTTP_REFERER'] = 'http://arachnid.org';
		$result = $Controller->referer(null, false);
		$expected = 'http://arachnid.org';
		$this->assertIdentical($result, $expected);

		$_SERVER['HTTP_REFERER'] = '';
		$result = $Controller->referer('http://arachnid.org', false);
		$expected = 'http://arachnid.org';
		$this->assertIdentical($result, $expected);
